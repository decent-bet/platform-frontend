function CGame(oData) {
    ...
    var isServerReelReady

    this._init = function() {
        var balances = parent.window.slotsController.balances()
        ...
        _iMoney = balances.user // When goToGame is called in CGame, this resets _iMoney's value to the current user balance from the Game component
        _iCurBet = MIN_BET
        ...
        _oFrontSkin = new createjs.Bitmap(
            s_oSpriteLibrary.getSprite('mask_slot')
        )
    ...
    this._initReels = function() {
        ...
        for (var i = 0; i < NUM_REELS; i++) {
            ...
            _aMovingColumns[i + NUM_REELS] = new CReelColumn(
                i + NUM_REELS,
                iXPos,
                iYPos + SYMBOL_SIZE * NUM_ROWS,
                iCurDelay
            )
    ...
    this.calculateWinningLine = function() {
        ...
        for (var k = 0; k < _iLastLineActive; k++) {
            ...
            while (iValue === WILD_SYMBOL && iStartIndex < NUM_REELS) {
                iNumEqualSymbol++
                iValue =
                    _aFinalSymbolCombo[aCombos[iStartIndex].row][
                        aCombos[iStartIndex].col
                    ]
                aCellList.push({
            ...
            for (var t = iStartIndex; t < aCombos.length; t++) {
                if (
                    _aFinalSymbolCombo[aCombos[t].row][aCombos[t].col] ===
                        iValue ||
                    _aFinalSymbolCombo[aCombos[t].row][aCombos[t].col] ===
                        WILD_SYMBOL
                ) {
            ...
            console.log(
                'Line ' +
                    k +
                    ': ' +
                    s_aSymbolWin[iValue - 1][iNumEqualSymbol - 1]
            )
        ...
        console.log(
            'Winning lines: ' + JSON.stringify(_aWinningLine) + ', ' + loops
        )
    }
    ...
    this.reelArrived = function(iReelIndex, iCol) {
        ...
        // }
        /**
         * Once result has arrived from the server,
         * call setReadyToStop and restart on _aMovingColumns[iReelIndex]
         **/
        if (_iCurReelLoops > MIN_REEL_LOOPS && isServerReelReady) {
            if (_iNextColToStop === iCol) {
                if (_aMovingColumns[iReelIndex].isReadyToStop() === false) {
                    ...
                    if (iReelIndex < NUM_REELS) {
                        ...
                        _aMovingColumns[iReelIndex].restart(
                            new Array(
                                _aFinalSymbolCombo[0][iReelIndex],
                        ...
                        _aMovingColumns[iReelIndex].restart(
                            new Array(
                                _aFinalSymbolCombo[0][iNewReelInd],
    ...
    this._endReelAnimation = function() {
        ...
        if (_aWinningLine.length > 0) {
            ...
            for (var i = 0; i < _aWinningLine.length; i++) {
                console.log('Winning line: ' + JSON.stringify(_aWinningLine[i]))
                _oPayTable.highlightCombo(
                    _aWinningLine[i].value,
                    _aWinningLine[i].num_win
                )
                _oInterface.showLine(_aWinningLine[i].line)
                var aList = _aWinningLine[i].list
                for (var k = 0; k < aList.length; k++) {
                    _aStaticSymbols[aList[k].row][aList[k].col].show(
                        aList[k].value
                    )
                }
            ...
            if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
                // Randomly plays one of the three sounds
                const soundArray = ['win', 'win2', 'win3']
                const soundIdx = Math.floor(Math.random() * soundArray.length)
                const soundToPlay = soundArray[soundIdx]
                _oCurSymbolWinSound = playSound(soundToPlay, 0.3, 0)
            }
        } else {
            _iCurState = GAME_STATE_IDLE
        }
    ...
    this.changeCoinBet = function() {
        ...
        if (iCurBetIndex === BET_SIZES.length - 1) iCurBetIndex = 0
        else iCurBetIndex++
    ...
    this.onSpin = function(lines, originalBalances) {
        if (originalBalances) {
            _iMoney = originalBalances.user
            _oInterface.refreshMoney(_iMoney)
            SLOT_CASH = originalBalances.house
        }

        if (_iCurState === GAME_STATE_SPINNING) {
            for (var i = _iNumReelsStopped; i < NUM_REELS * 2; i++)
                this.stopNextReel()
        }
        ...
        /** Replace the below with final symbols returned from the backend */
        _aFinalSymbolCombo = lines
        console.log('Final symbols: ' + JSON.stringify(_aFinalSymbolCombo))
        this.calculateWinningLine()
        setTimeout(function() {
            console.log('isServerReelReady: true')
            isServerReelReady = true
        }, 100)
