function CSlotSettings() {
    ...
    this._initSymbolAnims = function () {
        ...
        for (let index = 0; index < NUM_SYMBOLS; index++) {
            var oData = {
                framerate: 24,
                images: [s_oSpriteLibrary.getSprite(`symbol_${index+1}_anim`)],
                // width, height & registration point of each sprite
                frames: {width: SYMBOL_SIZE, height: SYMBOL_SIZE, regX: 0, regY: 0, count: 15},
                animations: {static: [0, 4], anim: [
                    // Create JS reads the sprites in vertical, but the animation is in horizontal.
                    0, 4, 8, 12,
                    1, 5, 9, 13,
                    2, 6, 10, 14,
                    3, 7, 11
            ...
            s_aSymbolAnims[index] = new createjs.SpriteSheet(oData);
        }
    }
