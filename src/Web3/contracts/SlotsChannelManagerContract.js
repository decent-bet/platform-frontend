...
export default class SlotsChannelManagerContract extends BaseContract {
    ...
    async logChannelActivate(channelId) {
        return new Promise((resolve, reject) => {
            this.instance.events.LogChannelActivate({
                filter: {
                    id: channelId
                }
            }).on('data', (data) => {
    ...
    async logChannelFinalized(id, fromBlock, toBlock) {
        return new Promise((resolve, reject) => {
            this.instance.events.LogChannelFinalized({
                filter: {
                    user: this._keyHandler.getAddress(),
                    id
                },
    ...
    async logClaimChannelTokens(id, fromBlock, toBlock) {
        return new Promise((resolve, reject) => {
            this.instance.events.LogClaimChannelTokens({
                filter: {
                    id
                },
                fromBlock: fromBlock ? fromBlock : 0,
    ...
    async logDeposit(fromBlock, toBlock) {
        return new Promise((resolve, reject) => {
            this.instance.events.LogDeposit({
                filter: {
    ...
    async logWithdraw(fromBlock, toBlock) {
        return new Promise((resolve, reject) => {
            this.instance.events.LogWithdraw({
                filter: {
                    _address: this._keyHandler.getAddress()
                },
                fromBlock: fromBlock ? fromBlock : 0,
