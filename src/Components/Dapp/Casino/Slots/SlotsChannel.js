...
class SlotsChannel extends Component {
    ...
    initWatchers = () => {
        const self = this
        let newChannelEvent = contractHelper.getGameChannelManagerInstance().NewChannel({}, {
            fromBlock: 0,
            toBlock: 'latest'
        })
        newChannelEvent.watch((err, event) => {
            if (err)
                ...
                if (event.transactionHash == self.state.lastChannelTx) self.toggleLoading(false)
                console.log('New Channel event: ' + JSON.stringify(event))
                let channel = event.args._address
                let user = event.args.user
                if (user == helper.getWeb3().eth.defaultAccount)
                    self.checkIfChannelExists(channel)
            }
        })
    }
        ...
        window.slotsController = () => {
            return {
                spin: (betSize, callback) => {
                    self.spin(betSize, callback)
                },
                balances: () => {
                    let lastHouseSpin = self.getHouseSpin(this.state.activeChannel.houseSpins.length - 1)
    ...
    checkIfChannelExists = (address) => {
        console.log('checkIfChannelExists: ' + address)
        const self = this
        contractHelper.getWrappers().gameChannelManager().getChannel(address).then((channel) => {
            let user = channel[0]
            let exists = channel[1]
            console.log('Retrieved channel: ' + user + ', ' + exists)
            if (exists)
                self.addToExistingChannels(address)
        })
    }

    addToExistingChannels = (address) => {
        let existingChannels = this.state.existingChannels
        existingChannels[address] = true
        this.setState({
    ...
    createChannel = () => {
        const self = this
        const deposit = 1000 * helper.getEtherInWei()
        console.log('Creating channel with deposit: ' + deposit)
        contractHelper.getWrappers().gameChannelManager()
            .createSlotsChannel(deposit).then((tx) => {
            self.toggleLoading(true, 'Creating channel..')
            self.setState({
                lastChannelTx: tx
            })
    ...
    getInitialRandomNumber = () => {
        return this.getRandomNumber(18) + ""
    }

    getUserHashes = (randomNumber) => {
        let hashes = []
        let aesKey = this.state.activeChannel.aesKey
    ...
    getExistingChannelItem = (address) => {
        const self = this
    ...
    joinChannel = (address) => {
        let aesKey = helper.getWeb3().sha3(address)
        helper.getWeb3().eth.sign(helper.getWeb3().eth.defaultAccount, aesKey, (err, sign) => {
            if (!err) {
                aesKey = sign
                if (!this.state.channelContracts[address]) {
                    const self = this
                    this.toggleLoading(true, 'Joining channel..')
                    contractHelper.getNewSlotsChannelContract(address, (err, contract) => {
        ...
        decentApi.getLastSpin(channel.address, (err, result) => {
            if (!err) {
                ...
                if (houseSpin)
                    ...
                    houseSpins = []
                console.log('Loaded spin: ' + nonce + ', ' + JSON.stringify(houseSpins))
    ...
    initChannelDetails = (channelContract, cb) => {
        async.parallel({
            ...
            finalUserHash: (callback) => {
                this.getFinalUserHash(channelContract, (err) => {
                    callback(false)
                })
            },
            finalReelHash: (callback) => {
                this.getFinalReelHash(channelContract, (err) => {
                    callback(false)
                })
            },
            finalReelSeedHash: (callback) => {
                this.getFinalReelSeedHash(channelContract, (err) => {
                    callback(false)
                })
            },
            ready: (callback) => {
                this.isUserReady(channelContract, (err) => {
                    callback(false)
                })
            },
            activated: (callback) => {
                this.isChannelActivated(channelContract, (err) => {
                    callback(false)
                })
            },
            initialDeposit: (callback) => {
                this.getInitialDeposit(channelContract, (err) => {
                    callback(false)
                })
            },
    ...
    getChannelAllowance = (channelContract, callback) => {
        const self = this
    ...
    getFinalUserHash = (channelContract, callback) => {
        const self = this
        let user = helper.getWeb3().eth.defaultAccount
    ...
    getFinalReelHash = (channelContract, callback) => {
        const self = this
    ...
    getFinalReelSeedHash = (channelContract, callback) => {
        const self = this
    ...
    isUserReady = (channelContract, callback) => {
        const self = this
        let user = helper.getWeb3().eth.defaultAccount
    ...
    isChannelActivated = (channelContract, callback) => {
        const self = this
    ...
    getInitialDeposit = (channelContract, callback) => {
        const self = this
        let user = helper.getWeb3().eth.defaultAccount
    ...
    getInitialUserNumber = (channelContract, callback) => {
        const self = this
        let user = helper.getWeb3().eth.defaultAccount
        channelContract.initialUserNumber().then((initialUserNumber) => {
            ...
            self.validateInitialUserNumber(initialUserNumber, activeChannel.finalUserHash)
            callback(false)
        }).catch((err) => {
    ...
    depositToChannel = () => {
        if (!this.state.activeChannel.ready) {
            const self = this

            let deposit = () => {
                let initialRandomNumber = self.getInitialRandomNumber()
                userHashes = self.getUserHashes(initialRandomNumber)
                ...
                self.getChannelContract().deposit.sendTransaction(initialUserNumber, finalHash, {
                    from: account,
                    gas: 3000000
                }).then((tx) => {
            ...
            console.log('Allowance: ' + this.state.activeChannel.allowance +
                ', ' + this.state.activeChannel.depositAmount)

            if (this.state.activeChannel.allowance < this.state.activeChannel.depositAmount) {
                /**
                 * Approve deposit amount on token contract before depositing to channel
                 * */
                contractHelper.getWrappers().token()
                    .approve(this.state.activeChannel.address,
                        this.state.activeChannel.depositAmount).then((tx) => {
    ...
    closeChannel = () => {
        ...
        betSize = this.fixBigNumber(betSize)
        this.getSpin(betSize, (err, spin) => {
            if (!err) {
                let priorSpin = self.state.activeChannel.houseSpins[self.state.activeChannel.houseSpins.length - 1]

                console.log('House spin: ' + JSON.stringify(priorSpin))
                console.log('Curr spin sign: ' + spin.sign)

                contractHelper.getWrappers().slotsChannel(self.getChannelContract())
    ...
    getChannelContract = () => {
        return this.state.channelContracts[this.state.activeChannel.address]
    }
    ...
    validateInitialUserNumber = (initialUserNumber, finalUserHash) => {
        ...
        userHashes = self.getUserHashes(initialUserNumber)
    ...
    spin = (betSize, callback) => {
        ...
        let address = this.state.activeChannel.address
        this.getSpin(betSize, (err, spin) => {
            if (!err) {
                decentApi.spin(address, spin, self.state.activeChannel.aesKey, (err, response) => {
                    ...
                    if (!err) {
                        self.verifyHouseSpin(response.message, spin, (err, message) => {
                            if (!err) {
                                ...
                                if (callback) {
                                    let lines = self.getLines(response.message.reel)
    ...
    getSpin = (betSize, callback) => {
        ...
        let lastHouseSpin = this.getHouseSpin(this.state.activeChannel.houseSpins.length - 1)
        let nonce = this.state.activeChannel.nonce

        let reelHash = (nonce == 1) ? this.state.activeChannel.finalReelHash : lastHouseSpin.reelHash
        let reel = ''
        let reelSeedHash = (nonce == 1) ? this.state.activeChannel.finalReelSeedHash : lastHouseSpin.reelSeedHash
        ...
        let userBalance = ((nonce == 1) ? (this.state.activeChannel.depositAmount) :
            lastHouseSpin.userBalance)
        let houseBalance = ((nonce == 1) ? (this.state.activeChannel.depositAmount) :
            lastHouseSpin.houseBalance)
        ...
        console.log('Tightly packed spin: ' + this.getTightlyPackedSpin(spin))

        decentApi.signString(this.getTightlyPackedSpin(spin), (err, sign) => {
            if (!err) {
                spin.sign = sign.sig
    ...
    getTightlyPackedSpin = (spin) => {
        return (spin.reelHash + (spin.reel != '' ? JSON.stringify(spin.reel) : '') +
    ...
    /**
     * Wrap all bytes32 variables to be passed into solidity function calls to support upto 64 characters.
     * They would need to be converted back on contract to their respective formats.
     * @param string
     * @returns {*}
     */
    wrapBytes32 = (string) => {
        if (typeof string == 'number')
            return helper.intToHex(string)
        else if (string == '')
            return '0x0'
        else
            return '0x' + string
    ...
    verifyHouseSpin = (houseSpin, userSpin, callback) => {
        ...
        async.waterfall([
             ...
             */
                ...
                let msg = new Buffer(self.getTightlyPackedSpin(nonSignatureSpin))

                const prefix = new Buffer("\x19Ethereum Signed Message:\n");
                let prefixedMsg = ethUtil.sha3(
                    Buffer.concat([prefix, new Buffer(String(msg.length)), msg])
                );
                prefixedMsg = ethUtil.bufferToHex(prefixedMsg)

                let sign = houseSpin.sign

                this.getChannelContract().checkSig.call(prefixedMsg, sign, houseSpin.turn).then((valid) => {
             ...
             */
                ...
                let payout = helper.convertToEther(self.calculateReelPayout(reel, betSize))
                ...
                userBalance = self.fixBigNumber(userBalance)
                houseBalance = self.fixBigNumber(houseBalance)
        ...
        switch (lineIndex) {
            case 0:
                for (let i = 0; i < NUMBER_OF_REELS; i++) {
                    line[i] = self.getSymbol(i, reel[i])
                }
                break
            case 1:
                for (let i = 0; i < NUMBER_OF_REELS; i++) {
                    line[i] = self.getSymbol(i, reel[i] - 1)
                }
                break
            case 2:
                for (let i = 0; i < NUMBER_OF_REELS; i++) {
    ...
    getLastReel = () => {
        ...
        if (houseSpins.length == 0)
            ...
            let reelString = ''
            for (let i = 0; i < reel.length; i++) {
                reelString += self.getSymbol(i, reel[i])
                if (i < reel.length - 1)
                    reelString += ', '
            }
            return reelString
    ...
    getHouseSpin = (index) => {
        return this.state.activeChannel.houseSpins[index]
    }
    ...
    getBalance = (player) => {
        if (this.state.activeChannel) {
            if (this.state.activeChannel.nonce == 1)
                return this.state.activeChannel.depositAmount
            else {
                if (this.state.activeChannel.houseSpins &&
                    this.state.activeChannel.houseSpins.length > 0) {
                    // console.log('Active channel: ' + JSON.stringify(this.state.activeChannel))
                    return helper.formatEther(this.getHouseSpin(this.state.activeChannel.houseSpins.length - 1)
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div className="row mt-5">
                            {   Object.keys(self.state.existingChannels).map((address) => {
                                return self.getExistingChannelItem(address)
                            })}
                        </div>
                        }
                    </div>
                    ...
                    <div style={{marginTop: 50}}>
                        ...
                        <div className="row mt-4">
                            <div className="col-12">
                                <h4 className="text-center">Last reel: { self.getLastReel() }</h4>
                            </div>
                            <div className="col-4">
                                <p className="text-center">User balance:
                                    { self.getBalance(PLAYER_USER) + ' DBETs'}</p>
                            </div>
                            <div className="col-4">
                                <p className="text-center">House balance:
                                    { self.getBalance(PLAYER_HOUSE) + ' DBETs'}</p>
                            </div>
                        </div>
